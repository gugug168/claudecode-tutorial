...
║  【中文概述】                                                      ║
╠══════════════════════════════════════════════════════════════════╣
║  这个命令是做什么的：PM2 进程管理初始化（完整版）                     ║
║  什么时候用它：需要为项目配置完整的 PM2 进程管理时                    ║
║  核心能力：服务检测、配置生成、命令文件生成、脚本生成                 ║
║  触发方式：/pm2 或通过 /setup-pm 初始化后使用                       ║
╚══════════════════════════════════════════════════════════════════╝
-->
...
<!--
【说明】服务检测：根据配置文件识别服务类型和默认端口
- Vite/Next.js/Nuxt/CRA：前端框架
- Express/Node：后端服务
...
- .claude/commands/：各种 PM2 命令文件
- .claude/scripts/：PowerShell 脚本
-->
## Generated Files
...
【说明】Windows 配置（重要）：
- 必须使用 .cjs 扩展名
- Node.js：指定 bin 路径 + 解释器
- Python：使用 Node.js 包装脚本 + windowsHide: true
-->
...
<!--
【说明】框架脚本路径：不同框架的 script 和 args 配置
-->
...
<!--
【说明】命令文件模板：每个文件只有 1-2 行描述 + bash 块
- pm2-all.md：启动所有 + 监控
...
<!--
【说明】执行：根据 $ARGUMENTS 执行初始化
1. 扫描项目服务
2. 生成 ecosystem.config.cjs
3. 为 Python 服务生成 start.cjs
4. 生成命令文件
5. 生成脚本文件
6. 更新项目 CLAUDE.md
7. 显示完成摘要
