...
║  【教学概述】                                                      ║
╠══════════════════════════════════════════════════════════════════╣
║  模板用途：Django REST API 项目配置模板                           ║
║  适合人群：使用 Django + DRF + PostgreSQL 的后端开发者           ║
║  核心要点：                                                        ║
║    - 领域驱动设计（DDD）按业务域组织应用                          ║
║    - 使用 Django ORM 防止 N+1 查询问题                            ║
║    - JWT 认证和权限控制                                           ║
║    - 服务层模式分离业务逻辑                                       ║
║    - pytest + Factory Boy 测试策略                                ║
╚══════════════════════════════════════════════════════════════════╝
-->

# Django REST API — 项目 CLAUDE.md

> Django REST Framework API 与 PostgreSQL 和 Celery 的真实示例。
...
**架构：** 领域驱动设计，按业务域组织应用。DRF 用于 API 层，Celery 用于异步任务，pytest 用于测试。所有端点返回 JSON - 不进行模板渲染。
...
- f-strings 用于字符串格式化，永远不用 `%` 或 `.format()`
- 使用 `pathlib.Path` 而不是 `os.path` 进行文件操作
- 使用 isort 排序导入：标准库、第三方、本地（由 ruff 强制执行）

### 数据库

- 所有查询使用 Django ORM - 原始 SQL 只能通过 `.raw()` 和参数化查询
- 迁移提交到 git - 永远不要在生产中使用 `--fake`
...
for order in orders:
    print(order.customer.name)  # 每个订单都访问数据库
...
- 当输入/输出形状不同时，分离读写序列化器
- 在序列化器级别验证，而不是在视图中 - 视图应该保持精简
